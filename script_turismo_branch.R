...
barplot (f_sexo, main = "Distribuição de frequências dos turistas segundo sexo", xlab = "sexo",
         ylab = "%")

f_estab = round(prop.table(table(dados$estabelecimento, useNA="no")) * 100, 2)

barplot (f_estab, main = "Distribuição de frequências dos turistas segundo tipo de
         estabelecimento de estadia", xlab = "tipo de estabelecimento", ylab = "%")

f_sexo_estab = round(prop.table(table(dados$sexo, dados$estabelecimento, useNA="no"),
                                 margin = 2) * 100, 2)

barplot(f_sexo_estab, main = "Distribuição dos turistas segundo sexo por tipo de
estabelecimento", xlab = "tipo de estabelecimento", ylab = "%", beside = TRUE)
